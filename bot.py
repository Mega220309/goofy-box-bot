...
bot = Bot(TOKEN)
dp = Dispatcher()
DB = pathlib.Path("products.json")
if not DB.exists(): DB.write_text("[]", encoding="utf-8")
def load():
    try: return json.loads(DB.read_text(encoding="utf-8"))
    except: return []
def save(d): DB.write_text(json.dumps(d, ensure_ascii=False, indent=2), encoding="utf-8")
def status(p):
...
async def a0(m:Message, state:FSMContext):
    ...
    await m.answer("📸 Кидай фото товара (можно до 10 штук, альбомом или по одной)\nКогда хватит - напиши ГОТОВО")
...
async def a_photo(m:Message, state:FSMContext):
    ...
    await m.answer(f"Принял {len(ph)} фото. Еще? Если все - напиши ГОТОВО")

@dp.message(Add.photos)
async def a_photos_text(m:Message, state:FSMContext):
    if m.text.lower() not in ["готово","готов","дальше","далее","done","ok"]:
        return await m.answer("Это не фото. Кидай фото или напиши ГОТОВО")
    d=await state.get_data()
    if not d.get("photos"):
        return await m.answer("Сначала кинь хоть 1 фото")
    await state.set_state(Add.name); await m.answer("Название? (например zip hoodie)")
...
async def a5(m:Message, state:FSMContext):
    await state.update_data(size=m.text.strip().upper()); await state.set_state(Add.desc); await m.answer("Описание товара? (ткань, состояние, замеры) - напиши - если без")

@dp.message(Add.desc)
async def a6(m:Message, state:FSMContext):
    txt="" if m.text.strip()=="-" else m.text.strip()
    await state.update_data(desc=txt); await state.set_state(Add.tag); await m.answer("Тэг? FRAGILE / 1 ШТ / - если без")
...
async def a7(m:Message, state:FSMContext):
    ...
    ps.insert(0, {
        ...
        "size":d["size"],"sizes":[d["size"]],
        "tag":tag,"description":d.get("desc",""),
        "img":d["photos"][0],"images":d["photos"],
        "sold":False,"sold_at":0
    })
    save(ps); await state.clear()
    await m.answer(f"✅ Добавил! Фото: {len(d['photos'])} шт\nВсего: {len(ps)}")

@dp.message(Command("list"))
async def lst(m:Message):
    if m.from_user.id!=ADMIN_ID: return
    for p in load()[:10]:
        kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔴 Продано" if not p.get("sold") else "↩️ Отменить", callback_data=f"{'sold' if not p.get('sold') else 'unsold'}_{p['id']}"), InlineKeyboardButton(text="🗑️", callback_data=f"del_{p['id']}")]])
...
async def api_products(r):
    ...
    for p in ps:
        if status(p)=="expired":
            try: await bot.send_message(ADMIN_ID, f"🗑️ УШЛО ЧЕРЕЗ 10Ч: {p['brand']} {p['name']} {p['price']}₽")
            except: pass
        else:
            keep.append(p); cp=p.copy(); cp["status"]=status(p); alive.append(cp)
