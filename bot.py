...
bot = Bot(TOKEN); dp = Dispatcher(); DB_FILE = "products.json"
...
async def add_start(m: Message, state: FSMContext):
    ...
    await state.set_state(Add.photo); await m.answer("📦 Кидай фото")
...
async def add_brand(m: Message, state: FSMContext):
    await state.update_data(brand=m.text); await state.set_state(Add.price); await m.answer("Цена?")
...
async def add_size(m: Message, state: FSMContext):
    await state.update_data(size=m.text.upper()); await state.set_state(Add.tag); await m.answer("Тэг? FRAGILE/1 ШТ или -")
@dp.message(Add.tag)
async def add_tag(m: Message, state: FSMContext):
    d=await state.get_data(); ps=get_products()
    ps.insert(0,{"id":int(time.time()*1000),"name":d["name"],"brand":d["brand"],"price":d["price"],"size":d["size"],"tag":"" if m.text=="-" else m.text,"img":d["photo"],"sold":False})
    save_products(ps); await state.clear(); await m.answer(f"✅ Добавил! Всего: {len(ps)}")
...
app.router.add_static('/', path=pathlib.Path(__file__).parent / 'static', show_index=True)

async def main():
    runner=web.AppRunner(app); await runner.setup(); site=web.TCPSite(runner,'0.0.0.0',8080); await site.start(); await dp.start_polling(bot)
